...
class Sendmail
{
    ...
    public $host = 'ssl://smtp.daum.net';
    /* smtp 계정 아이디 입력 */
    public $smtp_id = 'joeastgur';
    /* smtp 계정 비밀번호 입력 */
    public $smtp_pw = 'jodo3519';
    ...
    public function Sendmail($data = false)
    {
        if ($data != false) {
            if (is_array($data)) {
    ...
    /*  smtp 통신을 한다. */
    public function dialogue($code, $cmd)
    {
        fputs($this->fp, $cmd . "\r\n");
...
';
    ...
    /*  메시지를 보낸다. */
    public function smtp_send($email, $from, $data, $cc_mail, $bcc_mail, $rel_to = false)
    {
        $id = $this->smtp_id;
        $pwd = $this->smtp_pw;

        /* 이메일 형식 검사 구간*/
        if (!$mail_from = $this->get_email($from)) {
            return false;
        }
        if (!$rcpt_to = $this->get_email($email)) {
            return false;
        }
        ...
        if (!$this->dialogue(334, base64_encode($id))) {
            return false;
        }
        if (!$this->dialogue(235, base64_encode($pwd))) {
            return false;
        }
    ...
    public function attach($path, $name = '', $ctype = 'application/octet-stream')
    {
        if (is_file($path)) {
            $fp = fopen($path, 'r');
            $message = fread($fp, filesize($path));
            fclose($fp);
            $this->parts[] = ['ctype' => $ctype, 'message' => $message, 'name' => $name];
        } else {
            return false;
        }
    }
    ...
    public function build_data($subject, $body)
    {
    ...
    /* MX 값을 찾는다. */
    public function get_mx_server($email)
    {
        if (!preg_match("/([\._0-9a-zA-Z-]+)@([0-9a-zA-Z-]+\.[a-zA-Z\.]+)/", $email, $matches)) {
            return false;
        }
    ...
    /* 메일을 전송한다. */
    public function send_mail($to, $from, $subject, $body, $cc_mail = false, $bcc_mail = false)
    {
        $from .= ' <' . $this->smtp_id . '>';
    ...
    public function cc_email($rel_to, $from, $data, $cc_mail, $bcc_mail)
    {
        if (!is_array($cc_mail)) {
            $cc = explode(',', $cc_mail);
        }
    ...
    public function bcc_email($rel_to, $from, $data, $cc_mail, $bcc_mail)
    {
        if (!is_array($bcc_mail)) {
            $bcc = explode(',', $bcc_mail);
        }
