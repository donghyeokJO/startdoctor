<?php
  ...
  $user_query = 'select * from user where u_specify = "업체" or (u_specify = "미인증" and u_type is not null)';
    ...
    <section class="content">
      ...
      <div class="card">
            <div class="card-header">
              ...
              <button type="button" class="btn btn-block btn-danger btn-sm" style ="width:20%; margin-left:80%" onclick = "location.href='add_corp.html'">회원 추가</button>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                while ($row = mysqli_fetch_assoc($user_ret)) {
                    echo "
                <tr>
                  <td><a href = \"corp_detail.php?u_id=$row[u_id]\" style = \"color:#212529\">$row[u_name]</a></td>
                  <td>$row[u_specify]
                  </td>
                  <td>$row[u_email]</td>
                  <td>$row[u_phone]</td>
                ";
                    if ($row['u_specify'] == '미인증') {
                        echo "<td><button type=\"button\" class=\"btn btn-block btn-default btn-sm\" onclick = \"javascript:admit('{$row[u_id]}')\">업체회원 인증</button></td>";
                    } else {
                        echo '<td></td>';
                    }
                    echo '
...
<script>
  function admit(u_id){
    if(confirm("업체 회원으로 인증하시겠습니까?")==true){
      window.location = "user_corp.php?u_id=" + u_id;
    }
    else return;
  }
</script>
