<?php
  ...
  $id = $_GET['id'];
    $query = "select * from counsel natural join user where id='$id'";
    $ret = mysqli_query($conn, $query);
    $counsel = mysqli_fetch_assoc($ret);
    $query = "select * from re_counsel where id='$id'";
    ...
    <section class="content">
      ...
      <div class="card">
            <div class="card-header">
              <h3 class="card-title">스타트 닥터</h3>
              
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <div><b><h1>상담글<h1></b></div>
                <div><b>제목</b>: <?php echo $counsel['title']?></div>
                <div><b>유형</b>: <?php echo $counsel['type']?></div>
                <div><b>회원이름</b>: <?php echo $counsel['u_name']?></div>
                <div><b>내용</b>: <?php echo $counsel['content']?></div>
                <div><b>일시</b>: <?php echo $counsel['date']?></div>
            </div>
          ...
          <?php
            while ($row = mysqli_fetch_array($temp)) {
                ...
                $corp_name = mysqli_fetch_assoc($tmp)['u_name'];
                $cquery = "select * from coach where user1='$corp_id' and user2= '$counsel[u_id]'";

                $coach_id = mysqli_fetch_assoc(mysqli_query($conn, $cquery))['coach_id'];
                echo "
                <div class=\"card\">
            <div class=\"card-header\">
              <h3 class=\"card-title\">스타트 닥터</h3>
              <button type=\"button\" class=\"btn btn-block btn-primary btn-sm\" style =\"width:20%; margin-left:80%\"  onclick = \"location.href='show_chat.php?coach_id=$coach_id'\">채팅 내역 보기</button>
             
            </div>
            <!-- /.card-header -->
            <div class=\"card-body\">
                <div><b><h1>답변글<h1></b></div>
               
                
                <div><b>업체이름</b>:$corp_name</div>
                <div><b>내용</b>: $row[content]</div>
             
                <div><b>일시</b>:$row[date]</div>
