<?php
  ...
  $user_query = 'select * from user where u_specify = "관리자" or u_specify = "의사"';
            ...
            <div class="card-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr>
                  ...
                  <td>$row[u_phone]</td>
                  ";
                    if ($row['u_specify'] == '관리자') {
                        echo "<td><button type=\"button\" class=\"btn btn-block btn-default btn-sm\" onclick = \"javascript:disadmit('{$row[u_id]}')\">관리자 해제</button></td>";
                    } else {
                        echo "<td><button type=\"button\" class=\"btn btn-block btn-default btn-sm\" onclick = \"javascript:admit('{$row[u_id]}')\">관리자 인증</button></td>";
                    }
...
<script>
  function disadmit(u_id){
    if(confirm("관리자 권한을 박탈하시겠습니까?")==true){
      var key = prompt("사이트 관리 비밀번호를 입력하여 주세요: ");
      window.location = "user_downgrade.php?u_id="+u_id+"&key="+key;
    }
    else return;
  }
  ...
  function admit(u_id){
    if(confirm("관리자 권한을 부여하시겠습니까?")==true){
      var key = prompt("사이트 관리 비밀번호를 입력하여 주세요: ");
      window.location = "user_admin.php?u_id="+u_id+"&key="+key;
    }
    else return;
  }
</script>
