<?php
  ...
  $id = $_GET['id'];
  $login = true;
  $chat_query = "select * from counsel where id = '$id'";
  $chat_ret = mysqli_query($conn, $chat_query);
  $chat = mysqli_fetch_assoc($chat_ret);
  $reply_query = "select * from re_counsel where id ='$id'";
  $reply_ret = mysqli_query($conn, $reply_query);
  $reply_num = mysqli_num_rows($reply_ret);
    ...
    <section class="back_g_l">
      <article>
        <div class="container margin_top_l">
          <div class="row">
            <div class="col-12">
            <a href="service.php" style="text-decoration: none; color:#2b2b2b;">서비스</a> > <a href="chat.php" style="text-decoration: none; color:#2b2b2b;">상담 채팅</a>
            </div>
          ...
          <div class="row">
            <div class="col-12">
              <div class="container back_w padding_l">
                <div class="row">
                  <div class="col-12">
                    <div class="tag3" style="background-color: #efefef; color: #2b2b2b;"><?php echo $chat['type']?></div>
                  </div>
                  <div class="col-12 margin_top_l">
                    제목
                  </div>
            ...
            <div class="col-12 margin_top_l font_weight_b font_size_ll">
              답변<span class="margin_left_s color_b"><?php echo $reply_num?></span>
            </div>
            <?php
            while ($re = mysqli_fetch_assoc($reply_ret)) {
                $u_id = $re['corp_id'];
                $u_id2 = $row['u_id'];
                $query = "select * from user where u_id ='$u_id'";
                $ret = mysqli_query($conn, $query);
                $rere = mysqli_fetch_array($ret);
                $query = "select * from coach where user1='$u_id' and user2='$u_id2'";
                $eso = mysqli_query($conn, $query);
                $esos = mysqli_num_rows($eso);
                $esoo = mysqli_fetch_assoc($eso);
                if ($esos == 0) {
                    $query = "insert into coach(user1,user2) values('$u_id','$u_id2')";
                    mysqli_query($conn, $query);
                    $coach_id = mysqli_insert_id($conn);
                } else {
                    $coach_id = $esoo['coach_id'];
                }
                echo "
            <div class=\"container border_g padding_l\">
                <div class=\"row position_relative\">
                  <div class=\"col-12\">
                    <div class=\"display_inline_block profile\" style=\"background-image: url('../assets/default_profile.svg')\"></div>
                    <div class=\"display_inline_block font_size_l font_weight_b margin_left_s\">$rere[u_name]</div>
                  </div>
                  <div class=\"col-12 margin_top_m font_size_s\">";
                echo nl2br($re['content']);
                echo "
                   
                  </div>
                  <div class=\"col-12 font_size_s margin_top_l\">$re[date]</div>
                  <div class=\"position_absolute button small\" style=\"background-color: #2b2b2b; right:0; bottom: 0; padding: 0 15px;\"><a href = \"chat_list.php?coach_id=$coach_id\" style=\"color:#FFFFFF; text-decoration:none\">상담하러 가기</a></div>
                </div>
              </div>
          </div>
          ";
