<?php
  ...
  $query = "select * from notice where n_no = '$n_no'";

  $ret = mysqli_query($conn, $query);
  $notice = mysqli_fetch_assoc($ret);

  $num = $notice['n_hit'] + 1;

  $query = "update notice set n_hit = '$num' where n_no = $n_no";
...
<script>
  ...
  <div class="ploting_banner" onclick="location.href='https://pf.kakao.com/_xeIrFj'">
      ...
      <article>
        <div class="container">
          <div class="row align-items-center header_title_height">
            <div class="col-12 text_align_center color_w font_size_xl font_weight_b">공지사항</div>
          </div>
        </div>
      </article>
    </section>
    ...
    <section class="back_g_l" style="min-height: 100vh;">
      <article>
        <div class="container margin_top_xl" style="padding: 0 30px;">
          <div class="row justify-content-end">
            <div class="col-12">HOME > 공지사항</div>
          </div>
          <div class="row margin_top_m">
            <div class="col-12 back_w text_align_center padding_v_m border_top">
              <div class="font_size_ll font_weight_b"><?php echo $notice['n_title']?></div>
              <div class="margin_top_m">
                <span>관리자</span>
                <span class="margin_left_s margin_right_s">|</span>
                <span><?php echo $notice['n_date']?></span>
                <span class="margin_left_s margin_right_s">|</span>
                <span><?php echo $notice['n_hit']?></span>
              </div>
            </div>
          </div>
          <div class="row">
            <div class="col-12 back_w padding_l border_top border_bottom" style="min-height: 300px;">
              <?php echo nl2br($notice['n_content']) ?>
            </div>
          </div>

          <div class="row align-items-center justify-content-between">
            <div class="col-6 border_bottom padding_v_m ">
              <span class="cursor_pointer">❮</span>
              <span class="cursor_pointer">이전글</span>
              <?php
                $query = "select * from notice where n_no = $n_no-1";
                $res = mysqli_query($conn, $query);
                $previous = mysqli_fetch_array($res);
                $no = $n_no - 1;
                if (mysqli_num_rows($res) > 0) {
                    echo "<span class=\"cursor_pointer ellipsis font_weight_b margin_left_s\" style=\"width: 60%\"><a href=\"detail.php?n_no=$no\" style = \"color:#2B2B2B\">$previous[n_title]</a></span>";
                } else {
                    echo '<span class="cursor_pointer font_weight_b margin_left_s">없음</span>';
                }
              ?>
            </div>
            <div class="col-6 border_bottom padding_v_m text_align_right cursor_pointer">
              <?php
                $query = "select * from notice where n_no = $n_no+1";
                $res = mysqli_query($conn, $query);
                $no = $n_no + 1;
                $after = mysqli_fetch_array($res);
                if (mysqli_num_rows($res) > 0) {
                    echo "<span class=\"cursor_pointer ellipsis font_weight_b margin_right_s\" style=\"width: 60%\"><a href=\"detail.php?n_no=$no\" style = \"color:#2B2B2B\">$after[n_title]</a></span>";
                } else {
                    echo '<span class="cursor_pointer font_weight_b margin_right_s" style=\"width: 60%\">없음</span>';
                }
              ?>
              <span class="cursor_pointer">다음글</span>
              <span class="cursor_pointer">❯</span>
            </div>
          </div>

          <div class="row justify-content-center">
            <div class="col-12 col-sm-1 button margin_top_l" style="height: 38px;background-color: #2b2b2b;font-size: 14px;"><a href="notice.php" style="color:#FFFFFF">목록</div>
          </div>
        </div>
      </article>
    </section>
