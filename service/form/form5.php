<?php
   ...
   include_once $_SERVER['DOCUMENT_ROOT'] . '/PHPMailer/mailer.lib.php';
    ...
    $u_id = $_POST['u_id'];
    $f5_all = $_POST['f5_all'];
    $f5_extra = $_POST['f5_extra'];

    $query = "insert into form5(u_id,f5_all,f5_extra,date) values ('$u_id','$f5_all','$f5_extra',NOW())";
...
희망사항 : #{희망}';
    ...
    $content = str_replace('#{재직}', $f5_all, $content);
    // $content = str_replace('#{자격}', $f4_year, $content);
    // $content = str_replace('#{시기}', $f4_date, $content);
    $content = str_replace('#{희망}', $f5_extra, $content);
...
#{내용}';
        ...
        curl_close($ch);
        mailer('스타트닥터 관리자', 'austin@startdoctor.net', 'support@eszett.co.kr', '비교견적 - 의료장비 접수', '비교 견적 - 의료장비 접수 되었습니다. 확인해주세요.', 1);
