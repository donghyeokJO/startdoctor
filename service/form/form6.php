<?php
    ...
    $u_id = $_POST['u_id'];
    $f6_type = $_POST['f6_type'];
    $f6_detail = $_POST['f6_detail'];
    $f6_extra = $_POST['f6_extra'];

    $query = "insert into form6(u_id,f6_type,f6_detail,f6_extra,date) values ('$u_id','$f6_type','$f6_detail','$f6_extra',NOW())";
...
희망사항 : #{희망}';
    ...
    $content = str_replace('#{재직}', $f6_type, $content);
    $content = str_replace('#{자격}', $f6_detail, $content);
    // $content = str_replace('#{시기}', $f4_date, $content);
    $content = str_replace('#{희망}', $f6_extra, $content);
...
#{내용}';
        ...
        $msg = str_replace('#{카테고리}', '의료소모품, 세무사 및 기타', $msg);
        ...
        curl_close($ch);
        mailer('스타트닥터 관리자', 'austin@startdoctor.net', 'support@eszett.co.kr', '비교견적 - 기타 접수', '비교 견적 - 기타 접수 되었습니다. 확인해주세요.', 1);
