<?php
    ...
    $u_id = $_POST['u_id'];
    $f4_money = $_POST['f4_money'];
    $f4_extra = $_POST['f4_extra'];
    $f4_type = $_POST['f4_type'];
    $f4_date = $_POST['f4_date'];
    $f4_year = $_POST['f4_year'];

    $query = "insert into form4(u_id,f4_money,f4_type,f4_date,f4_extra,f4_year,date) values ('$u_id','$f4_money','$f4_type','$f4_date','$f4_extra','$f4_year',NOW())";
...
희망사항 : #{희망}';
    ...
    $content = str_replace('#{재직}', $f4_type, $content);
    $content = str_replace('#{자격}', $f4_year, $content);
    $content = str_replace('#{시기}', $f4_date, $content);
    $content = str_replace('#{희망}', $f4_extra, $content);
    if (!$ret) {
...
#{내용}';
        ...
        curl_close($ch);
        mailer('스타트닥터 관리자', 'austin@startdoctor.net', 'support@eszett.co.kr', '비교견적 - 자금대출 접수', '비교 견적 - 자금대출 접수 되었습니다. 확인해주세요.', 1);
